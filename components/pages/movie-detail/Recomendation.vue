<template>
  <div class="container mx-auto py-16">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
      <MovieCard v-for="movie in data.results.slice(0, 5)" :rating="movie.vote_average.toString()" :title="movie.title"
        :year="$dayjs(movie.release_date).format('YYYY')" :to="`/movies/${movie.id}`"
        :image-path="TMDB_IMAGE_BASE_PATH + movie.poster_path" :genre="genreStore.getMappedGenres(movie.genre_ids, 1)" />
    </div>
  </div>
</template>
